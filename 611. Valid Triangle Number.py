...
class Solution(object):
    def triangleNumber(self, nums):
        ...
        """
        dct = Counter(nums)
        unique = sorted(k for k in dct if k > 0)
        n = len(unique)
        unique_count = [dct[u] for u in unique]
        cum_count = unique_count[:]
        for i in range(1, n):
        ...
        for i, k in enumerate(unique):
            ...
            if c > 2:
                count += c * (c - 1) * (c - 2) // 6
            if c > 1:
                pos = bisect.bisect_left(unique, k<<1, lo=i+1) - 1
                count += c * (c - 1) // 2 * (cum_count[pos] - c)

        for i in range(2, n):
            for j in range(i - 1, 0, -1):
                t = unique_count[i] * unique_count[j]
                for k in range(j - 1, -1, -1):
                    if unique[j] + unique[k] > unique[i]:
                        count += t * unique_count[k]
                    else:
                        break
                if k + 1 == j:
                    break
